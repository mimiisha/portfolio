...
const AboutMe = () => {
  const KnowledgeItem = ({ name, Icon }) => (
    <div className="flex flex-col items-center text-base md:text-xl ">
      {name}
      <Icon className="transition-all duration-300 hover:text-cyan-700 w-14 h-14 md:w-24 md:h-24" />
    </div>
  )
    ...
    <div>
      ...
      <div className="bg-black">
        <div className="my-10 mx-16 flex flex-col gap-8 py-8">
          <h1 className="md:text-4xl font-bold text-cyan-400 flex justify-center md:justify-start text-3xl">Conhecimentos</h1>
          <section className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-y-8 text-xl text-cyan-500 justify-items-center items-center">
